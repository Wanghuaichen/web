            ...
            tablePaging: function (options) {
                ...
                $(this).each(function () {
                    ...
                    if (allPage > 1) {
                        ...
                        jQuery(queue).append('<p class="paging"><a id="prePage" tableid="'
                            + jQuery(this).attr('id') + '" pageindex='
                            + (parseInt(jQuery(this).data("pageindex")) - 1)
                            + ' href="#">previous</a> | '
                            +(parseInt(jQuery(this).data("pageindex")) + 1)+'/' + allPage
                            + ' | <a id="nextPage" tableid="' + jQuery(this).attr('id') + '" pageindex=' + (parseInt(jQuery(this).data("pageindex")) + 1) + ' href="#">next</a>'
                            + '<input type="text" size="4" maxlength="3" id="currentPage" /><input type="button" id="goPage" value="go"/></p>');
                        ...
                        jQuery("#" + jQuery(this).attr('id') + "Paging").find("#goPage").bind("click",settings,function(e){
                           //enter
                           // if(e.keyCode == 13){
                            var test = jQuery(this).val();
                            var toPageIndex = parseInt(test);
                            if (toPageIndex < 0) toPageIndex = allPage - 1;
                            if (toPageIndex >= allPage) toPageIndex = 0;
                            jQuery("#" + jQuery(this).attr("tableid")).data("pageindex", toPageIndex);
                            var data = e.data;
